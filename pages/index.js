    ...
    <div className="p-4 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <select className="w-full border p-2 rounded" value={service} onChange={(e) => setService(e.target.value)} required>
        ...
        <div>
          ...
          {aiSuggestion && <p className="text-sm text-green-600 mt-2">AI Suggestion: {aiSuggestion}</p>}
        </div>
        <div>
          <label className="block font-medium mb-1">Briefly describe the issue</label>
          <textarea className="w-full border p-2 rounded" rows={4} value={description} onChange={(e) => setDescription(e.target.value)} required></textarea>
        </div>
